...
namespace Aws.GameLift.Server
{
    public partial class AuxProxyMessageSender
    {
        ...
        private static void SetFutureErrorResult<T>(TaskCompletionSource<T> future, object response) where T : GenericOutcome
        {
            try
            {
                var deserialized = JsonConvert.DeserializeObject<GameLiftResponse>(response as string);
                switch (deserialized.Status)
                {
                    case GameLiftResponseStatus.ERROR_400:
                        ...
                        break;
                    case GameLiftResponseStatus.ERROR_500:
