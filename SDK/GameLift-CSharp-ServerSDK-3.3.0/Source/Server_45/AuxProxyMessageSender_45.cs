...
namespace Aws.GameLift.Server
{
    public partial class AuxProxyMessageSender
    {
        ...
        private void SetFutureErrorResult<T>(TaskCompletionSource<T> future, object response) where T : GenericOutcome
        {
            try
            {
                var deserialized = Com.Amazon.Whitewater.Auxproxy.Pbuffer.GameLiftResponse.Parser.ParseJson(response as string);
                switch (deserialized.Status)
                {
                    case Com.Amazon.Whitewater.Auxproxy.Pbuffer.GameLiftResponse.Types.Status.Error400:
                        future.TrySetResult((T)new GenericOutcome(new GameLiftError(GameLiftErrorType.BAD_REQUEST_EXCEPTION, deserialized.ErrorMessage)));
                        break;
                    case Com.Amazon.Whitewater.Auxproxy.Pbuffer.GameLiftResponse.Types.Status.Error500:
                        future.TrySetResult((T)new GenericOutcome(new GameLiftError(GameLiftErrorType.INTERNAL_SERVICE_EXCEPTION, deserialized.ErrorMessage)));
                        break;
                    default:
                        future.TrySetResult((T)new GenericOutcome(new GameLiftError(GameLiftErrorType.SERVICE_CALL_FAILED, deserialized.ErrorMessage)));
                        break;
                }
            }
            catch (Exception)
            {
                future.TrySetResult((T)new GenericOutcome(new GameLiftError(GameLiftErrorType.SERVICE_CALL_FAILED)));
            }
        }
        ...
        T EmitEvent<T>(IMessage message, IAck ackFunction, TaskCompletionSource<T> future, T error) where T : GenericOutcome
        {
            ...
            lock (emitLock)
            {
                socket.Emit(message.Descriptor.FullName, ackFunction, message.ToByteString().ToBase64());
            }
